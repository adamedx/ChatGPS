...
.DESCRIPTION
Register-ChatPlugin allows users to define new chat plugins and make them available for use in chat sessions; the language model can use the plugin, which contains PowerShell Script Block code that you provide, to direct ChatGPS to execute those script blocks to supply information to the language model or perform actions on the computer system according to instructions from the language model. For instance you could create a plugin that uses a PowerShell Script Block to return the list of operating system updates applied to the computer in a given time period; once added to a session, that plugin can be utilized to answer the user's conversational queries about such updates.

ChatGPS provides several built-in plugins from Semantic Kernel, and Register-ChatPlugin enables you to define any number of custom plugins with arbitrary functionality implemented as PowerShell code. Once registered, these custom plugins behave in almost all ways just like the built-in plugins in that they can be added to any session with Add-ChatPlugin or Connect-ChatSession, configured for a session using ChatGPS session settings, and enumerated through the Get-ChatPlugin -ListAvailable command. For more details on chat plugins and the features they provide, see the documentation for Add-ChatPlugin.

Chat plugins are defined as a collection of one or more plugin functions, and to create a plugin with Register-ChatPlugin, you must supply a collection of plugin functions created by the Add-ChatPluginFunction command; Add-ChatPluginFunction enables you to create a plugin function from a PowerShell Script Block. Register-ChatPlugin can accept any number of plugin functions created by the Add-ChatPluginFunction through the pipeline as a way to specify the collection.

Once a plugin is registered, it cannot be changed in any way. However it can be unregistered using the Unregister-ChatPlugin command. A new plugin with the name of the originally registered plugin can then be registered with Register-ChatPlugin as a way of "re-registering" the plugin.

If a given session's configuration is saved through the Save-ChatSessionSetting comand and that session has a custom plugin registered through Register-ChatPlugin, the session configuration will include the definition of the plugin. When the configuration is initialized in the future (e.g. when the ChatGPS module is loaded), the session will be re-registered and can be used just as it can immediately after the invocation of Register-ChatPlugin. This makes it easy to define such plugins just once and use them on an ongoing basis.
...
.PARAMETER Function
A plugin function created by the Add-ChatPluginFunction command. The plugin function encapsulates a PowerShell Script Block that can be executed to perform a function requested by the language model. The plugin function's name, description, and other information are used by language models to determine when to invoke the function. Note that this parameter receives pipeline input, so use the pipeline to to specify more than one plugin function for the plugin.
...
In this example, Register-ChatPlugin creates a new plugin called system_basic_information from a single chat function piped to it as input; this function simply returns the output of the Get-Uptime command. After the plugin is registered, the Add-ChatPlugin command must still be used to add it to a session (in this case the current session). The subsequent invocation of Send-ChatMessage returns a response from the language model with a friendly description, and its output can be compared with that of the next command invocation which actually executes Get-Uptime interactively in the shell. Note that both the response from Send-Message and invocatio of Get-Uptime return the same information, indicating that the language model did indeed choose the system_uptime function for invocation and its output was used successfully to generate the final response from the language model.
