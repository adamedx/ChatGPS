...
The function to process may by specified to Invoke-ChatFunction by its unique identifier or for functions that have a user defined name the name may also be specified. Chat functions can take parameters, so the Parameters parameter of Invoke-ChatFunction is used to specify any parameters for the function.
...
.PARAMETER CustomRunBlock
Specifies that a script block that will be used to wrap the code. This script block serves the same purpose as the automatically generated wrapper specified by the RunBlock parameter; see that parameter description for more details about run blocks. The wrapper script specified to CsutomRunBlock can be used to provide a PowerShell wrapper that can execute the generated code for languages other than PowerShell. The script block must take a single string parameter which will be the code that is generated for the language given by Language; the script block can then pass that code to the appropriate language runtime capable of executing it.
...
.PARAMETER GenerateRunBlock
Specifies that the command should generate a PowerShell script block that wraps the generated code; the script block will attempt to execute the code using a runtime appropriate for the generated language. This parameter is currently only supported when the Language parameter specifies the Python or Javascript languages, and requires that the system has the given language runtime installed and accessible from PowerShell; it is not guaranteed to work with all versios of those languages. This parameter is ignored when generating PowerShell code since it is unnecessary, and also has no effect if the given language is not supported by GenerateRunBlock. To add a wrapper for an unsupported language, see the CustomRunBlock parameter.
...
This example is much like that previous case, except the generated code is output to a PowerShell script file. The file is then executed. Note that even if you remove ChatGPS from the system, the generated code will still function as it does not depend on ChatGPS (nor on language models in general).
...
Note that this generated code for non-PowerShell languages does not provide a way to send arbitrary input to the target language runtime. To solve the parameter passing problem for non-PowerShell languages, workarounds include directing the code generator to create code that reads input from environment variables or a file, or generating PowerShell code with code embedded in the non-PowerShell language and using the Powershell code generate a string of the target language at runtime that invokes the embedded code with the values of parameters passed to the outer PowerShell script block's parameters.Kthat uses the parameters passed in the PowerShell code's script block.
...
function Build-ChatCode {
    ...
    param(
        ...
        [scriptblock] $CustomRunBlock = $null,
    ...
    process {
        ...
        if ( ! $generationResult.GenerationException ) {
            ...
            $boundCode = if ( $GenerateRunBlock.IsPresent -or $CustomRunBlock ) {
                GetBoundCode $generatedCode $Language $CustomRunBlock $NoScriptBlock.IsPresent
            } else {
                $generatedCode
            }
