...
public class ChatHistory : IList<ChatMessageContent>, IReadOnlyList<ChatMessageContent>
{
    ...
    {
        VerifyNotNullOrWhiteSpace(message);
    ...
    public void AddMessage(AuthorRole authorRole, string content, IReadOnlyDictionary<string, object?>? metadata = null) =>
        this.Add(new ChatMessageContent(authorRole, content, metadata));

    public void AddMessage(AuthorRole authorRole, ChatMessageContentItemCollection contentItems, IReadOnlyDictionary<string, object?>? metadata = null) =>
        this.Add(new ChatMessageContent(authorRole, contentItems, metadata));
    ...
    private void VerifyNotNull(object? item)
    {
        if ( item is null )
        {
            throw ArgumentException("null objects are forbidden");
        }
    }

    private void VerifyNotNullOrWhitespace(object? item)
    {
        VerifyNotNull(item);
    }
}
